...
class AdminController extends Controller
{
    ...
    public function users()
    {
        ...
        $users = User::orderBy('id', 'DESC')->with('roles', 'wallet')->paginate(1000);
        $count = $users->total();
 ...
 public function cobranca()
    {
        ...
        $roles = Role::all();
        return view('admin.cobranca', array(
            'count' => $count,
            'users' => $users,
            'roles' => $roles,
        ));
    ...
    {
        $users = User::role('Delivery Guy')->orderBy('id', 'DESC')->with('roles')->paginate(800);
        $count = $users->total();
    ...
    {
        $orders = Order::orderBy('id', 'DESC')->with('accept_delivery.user', 'restaurant')->paginate(1000);
        $count = $orders->total();
    ...
    public function dinheiro()
    {
        $orders = Order::orderBy('id', 'DESC')->with('accept_delivery.user', 'restaurant')->paginate(1000);
        $count = $orders->total();
        return view('admin.dinheiro', array(
            'orders' => $orders,
            'count' => $count,
        ));
    }
